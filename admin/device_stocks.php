 ...
 <br/>
	<div class="pull-left">
			 ...
			 <?php	
               ...
               ?>
				<li class="">
				<a href="cpu.php">WORKSTATION&nbsp;<span class="label label-default"> <?php echo $count;?></span></a>
				</li>
			 ...
			 <?php	
	           ...
	           $count_item=mysql_query("select * from stdevice 
			   LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
			   where dev_name = 'Video Graphic Accelerator (VGA)'
			   OR dev_name = 'VGA'
			   OR dev_name = '(VGA)'
			   OR dev_name = 'Video.Graphic.Accelerator' 
			   OR dev_name = 'Video-Graphic-Accelerator' 
			   OR dev_name = 'Video_Graphic_Accelerator'
			   OR dev_name = 'VideoGraphicAccelerator' 
			   OR dev_name = 'Video Graphic' 
			   OR dev_name = 'Vedio Graphic Acelerator' 
			   OR dev_name = 'VIDEO GRAPHIC ACCELATOR'
			   OR dev_name = 'VIDEO+GRAPHIC+ACCELATOR' 
			   OR dev_name = 'vido grphic' 
			   OR dev_name = 'viDo gRphic Accelarator' 
			   OR dev_name = 'video graphic accelarator'
			   OR dev_name = 'vedeo graphic' 
			   OR dev_name = 'video graphic accelerator' 
			   OR dev_name = 'videographicaccelerator'
			   OR dev_name = 'vedio graphic accelerator'
			   ORDER BY stdevice.id DESC");
	           $count = mysql_num_rows($count_item);
               ?>
				<li class="">
...
<?php 				/* ini Contoh Fungsi untuk medapatkan detail lokasi */
						...
						while($row2 = mysql_fetch_array($my_device)){
						$iddevice = $row2['id']; 	
						...
						LEFT JOIN stdev_location_name ON stlocation.stdev_id=stdev_location_name.stdev_id where stlocation.stdev_id = '$get_id' ")or die(mysql_error());
						}
...
?>
		...
		<td><?php
			...
			<td class="empty" width="80">
				<a rel="tooltip"  title="Edit Device" id="e<?php echo $id; ?>" href="device_edit.php<?php echo '?id='.$id; ?>" class="btn btn-success"><i class="icon-pencil"></i></a>

				<a rel="tooltip"  title="Delete Device" id="d<?php echo $id; ?>" href="device_stocks.php?stdev_id=0<?php echo '?id='.$id; ?>" data-toggle="tooltip" title="Delete!" class="btn btn-danger"><i class="icon-trash"></i>
				<?php
					if (isset($_POST['d'])){
					$id=$_POST['d'];
					$N = count($id);
					...
					header("location: device_stock.php?stdev_id=0");
					}
					?>
				</a>
