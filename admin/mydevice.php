            ...
            <div id="block_bg" class="block">
                  <div class="navbar navbar-inner block-header">							
							  <div class="muted pull-left"><i class="icon-building"></i>  <?php echo $location_row['stdev_location_name']; ?> </div>
								...
								Total Number of Device On Location: <span class="badge badge-info"><?php echo $count_my_device; ?></span>
								</div>								
                    </div>
             ...
             (select * from location_details where dev_status='Dump')
			  ...
			  <?php	
               ...
               ?>
				...
				where NOT EXISTS 
				(select * from location_details where dev_status='Dump') 
			    and dev_name = 'Workstation'   and stdev_id = '$get_id' 
				OR NOT EXISTS 
				(select * from location_details where dev_status='Dump') 
			    and dev_name = 'Mac Pro'   and stdev_id = '$get_id' 
				OR NOT EXISTS
				(select * from location_details where dev_status='Dump') 
			    and dev_name = 'PC'   and stdev_id = '$get_id' 
				OR NOT EXISTS
                (select * from location_details where dev_status='Dump') 
               ...
               ?>
				<li class="">			
				    <a href= 'mycpu.php<?php echo '?stdev_id='.$get_id; ?>')">WORKSTATION&nbsp;<span class="label label-default"> <?php echo $count;?></span></a>					
				</li>
               ...
               ?>				
				...
				where NOT EXISTS 
                (select * from location_details where dev_status='Dump')
				 and dev_name = 'Capture Card 'and stdev_id = '$get_id'
				 OR NOT EXISTS                 
                (select * from location_details where dev_status='Dump')
				and dev_name = 'CAPTURE CARD'      and stdev_id = '$get_id'
			    ")or die(mysql_error());
               ...
               ?>				
				<li class ="">			
				    <a href= 'myavr.php<?php echo '?stdev_id='.$get_id; ?>')">CAPTURE CARD&nbsp;<span class="label label-default"> <?php echo $count;?></span></a>					
				</li>
...
<tr>
		<td><?php
			...
			<!--
			<td width="185" class="btn-block">
			<a rel="tooltip"  title="Transfer Device" id="e<?php echo $id; ?>" href="transfer.php<?php echo '?id='.$id; ?>&<?php echo 'stdev_id='.$get_id; ?>" class="btn btn-warning"><i class="icon-move"></i> Transfer </a>
			<a rel="tooltip"  title="Update Status" id="t<?php echo $id; ?>" href="devl_update_status.php<?php echo '?id='.$id; ?>&<?php echo 'stdev_id='.$get_id; ?>" class="btn btn-success"><i class="icon-upload-alt"></i> Status</a>	
			<a rel="tooltip"  title="Edit Device" id="e<?php echo $id; ?>" href="device_edit2.php<?php echo '?id='.$id; ?>" class="btn btn-primary"><i class="icon-pencil"> Edit &nbsp &nbsp &nbsp  </i></a></td>	
			-->
			<td width="80">
			<div class="btn-group" role="group"> 
    		<button id="btnGroupDrop1" type="button" class="btn-block btn-danger btn-block dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" >
			<i class="icon-arrow-down"></i>	 Action &nbsp &nbsp &nbsp
    		</button> 
    		<div class="dropdown-menu" aria-labelledby="btnGroupDrop1" > 

			<div>
			<a rel="tooltip"  title="Transfer Device" id="e<?php echo $id; ?>" href="transfer.php<?php echo '?id='.$id; ?>&<?php echo 'stdev_id='.$get_id; ?>" class="btn btn-link"><i class="icon-move"></i> Transfer </a>
			</div>

			<div>
			<a rel="tooltip"  title="Update Status" id="t<?php echo $id; ?>" href="devl_update_status.php<?php echo '?id='.$id; ?>&<?php echo 'stdev_id='.$get_id; ?>" class="btn btn-link"><i class="icon-upload-alt"></i> Status &nbsp &nbsp</a>
			</div>
			   
			<div>   
			<a rel="tooltip"  title="Edit Device" id="e<?php echo $id; ?>" href="device_edit2.php<?php echo '?id='.$id; ?>" class="btn btn-link"><i class="icon-pencil"> Edit &nbsp &nbsp &nbsp &nbsp </i></a></td>
			</div>
